...
oo::class create www::connection {
	...
	if {[dict get $opts -code] == 0} {
	    ...
	    if {$transform ne ""} {
		if {[catch {{*}$transform $fd $host} err opts]} {
		    log "Transform: $err"
		}
	    }
	    return 1
	...
	my Return $result
	if {[package vsatisfies $version 1.1] && "close" ni $connection \
	  || ![my Close]} {
	    my Process
	}
    }
    ...
    method Close {} {
	my variable pending waiting
	my Disconnect
	set done [expr {[llength $pending] == 0 && [llength $waiting] == 0}]
	if {$done} {
	    # Nothing left to do
	    my destroy
	}
...
proc www::request {method url request args} {
    ...
    if {$code in {101 200 201 202 204 304}} {
